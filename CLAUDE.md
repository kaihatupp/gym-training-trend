...
- **フェーズ3(完了)**: 消費カロリーの概算表示(体重×METs値×時間の簡易計算)、摂取カロリー目安(Mifflin-St Jeor式によるBMR推定。維持/減量目安(-500kcal)/増量目安(+300kcal)の3値)、摂取カロリー実績の記録。いずれの概算値も「一般的な計算式による概算であり医学的指導ではない」旨を明記している。
  ...
  - **「あと◯◯kcalまでOK」/「◯◯kcalオーバー」の計算方法を修正(2026-08-28)**。摂取カロリー目安(維持/減量/増量)自体は基礎代謝×1.2のみの固定値とし(その日の運動量で変動しない)、「あと◯◯kcalまでOK」の算出時に**その日の運動による消費カロリーを別途加算**する形にした(残り = 減量目安 + 本日の消費カロリー − 摂取カロリー実績)。以前は目安の値自体に運動分を内包していたため、運動を考慮していないように見えるという指摘を受けての変更。
