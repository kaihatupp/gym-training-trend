...
- **フェーズ3(完了)**: 消費カロリーの概算表示(体重×METs値×時間の簡易計算)、摂取カロリー目安(Mifflin-St Jeor式によるBMR推定。維持/減量目安(-500kcal)/増量目安(+300kcal)の3値)、摂取カロリー実績の記録。いずれの概算値も「一般的な計算式による概算であり医学的指導ではない」旨を明記している。
  ...
  - 撤去した食事例の代わりに、**実際に食べた量を記録する「摂取カロリー実績」機能を追加(2026-08-28)**。朝食/昼食/夕食/間食ごとに、プロフィールに登録した「よく使うメニュー」を選び、**数量を指定して追加**できる(例: 焼酎1杯=100kcalを3杯選ぶと300kcalとして追加)。複数のメニューを積み上げて合計することも可能(例: 昼食=お弁当550kcal+ゆで卵120kcal=670kcal)。登録がないものはkcalを直接入力して追加することもでき、追加した項目は一覧表示され個別に削除できる。消費カロリー概算・摂取カロリー(実績・全食事の合計)・摂取カロリー目安(維持/減量/増量)を並べて表示している。推移の表示が必要になった場合もフェーズ2の反省を踏まえグラフ化はせず、数値・表形式で対応する方針。
  - **「あと◯◯kcalまでOK」/「◯◯kcalオーバー」の計算方法を修正(2026-08-28)**。摂取カロリー目安(維持/減量/増量)自体は基礎代謝×1.2のみの固定値とし(その日の運動量で変動しない、運動量を加味しない上限値として見たいというユーザーの要望通り)、「あと◯◯kcalまでOK」を**運動分を加味した数値**(残り = 減量目安 + 本日の消費カロリー − 摂取カロリー実績)と**運動分を加味しない数値**(残り = 減量目安 − 摂取カロリー実績)の2種類を並べて表示するようにした。
...
- プロフィール画面で1箇所管理(筋トレマシンと同じ運用)。名前とkcalを登録しておき、記録画面では登録済みリストから数量を指定して選ぶだけで済むようにする。
- 初期登録データは持たず、ユーザーが手動で登録する(自動投入を試したが実機で反映されずうまくいかなかったため、2026-08-28に撤回)。
...
// MealLogEntry:     { mealType('breakfast'|'lunch'|'dinner'|'snack'), kcal, mealItemId?, qty?, memo? }
//                    // 1食につき複数件になり得る(例: 昼食=お弁当+ゆで卵の2件)。kcalは数量を掛けた後の合計値、
//                    // qtyは表示用の内訳(例: 焼酎×3)。qtyを省略した場合は1件=1個として扱う。
// WorkoutLog: {
//   id, date(YYYY-MM-DD),
//   bodyWeightKg,                         // その日の体重
...
1. **今日の記録(ホーム)**: 曜日に応じたテンプレートを自動表示。体重、有酸素運動(距離・時間・傾斜有無、プロフィールの基本値を自動入力)、筋トレ(登録済みマシンから選択、重量はプロフィール値が自動入力・上書き可)、ストレッチ(月曜のみ)、今日の食事(朝食/昼食/夕食/間食ごとに、登録済みの「よく使うメニュー」を選び数量を指定して追加(例: 焼酎×3)、複数メニューを積み上げて合計も可能、登録がないものはkcal直接入力で追加可能。追加した項目は個別に削除できる)、推定カロリー(消費カロリー概算・摂取カロリー(実績)・摂取カロリー目安(維持/減量/増量、運動量を加味しない固定値)を表示し、さらに減量目安までの残りを運動分を加味した数値/加味しない数値の2種類で「あと◯◯kcalまでOK」/「◯◯kcalオーバー」表示、入力に応じてリアルタイム再計算)を入力して保存。
2. **記録一覧**: 日付順に過去の記録を一覧表示(14件超で「もっと見る」に折りたたみ)。タップして詳細確認・編集・削除。下部にバックアップ(エクスポート/インポート、記録+プロフィールをまとめてJSON化)。
3. **サマリー**: 連続記録日数(現在/自己ベスト)、週次・月次サマリー(前へ/次へで期間移動、**トレーニング記録**(記録日数・有酸素運動・筋トレ・ストレッチ)と**カロリーサマリー**(平均体重・消費カロリー合計・摂取カロリー合計・減量目安オーバー日数(運動分を加味/運動分含まずの2種類)の5項目)を分けて表示・2026-08-28に分離)、成長の横断比較(マシンごとの初回→最新の重量・回数比較表)。**アクティブな登録マシンは、まだ有効な記録がなくても表(「記録なし」)に表示されるようにしている(2026-09-14修正。以前は有効な重量記録が1件もないマシンが表から除外されており、新規登録直後のマシンが横断比較に出てこないという不具合があった)。**
...
**保存し忘れ防止(2026-08-28追加)**: 「今日の記録」フォームを編集後、未保存のまま他タブへ切り替えたり日付を変更したりしようとすると確認ダイアログを表示し、誤って入力内容を失わないようにしている(ブラウザを閉じる/リロードする場合も同様に確認が入る)。

**ストレッチの「実施した」チェックボックス(2026-09-14追加)**: ストレッチをやらない日もあるため、時間(分)の入力欄とは別に「実施した」チェックボックスを追加した。曜日テンプレートでストレッチを表示する日はデフォルトでチェック済み・時間欄もいつもの時間が自動入力されるが、その日やらなかった場合はチェックを外すだけで良い(時間欄の数値を消さなくても、チェックが外れていれば`stretch.durationMin`はnullとして保存され、消費カロリー概算にも含まれない)。筋トレのマシン選択チェックボックスと同じ考え方。
