...
- **フェーズ3(完了)**: 消費カロリーの概算表示(体重×METs値×時間の簡易計算)、摂取カロリー目安(Mifflin-St Jeor式によるBMR推定。維持/減量目安(-500kcal)/増量目安(+300kcal)の3値)、摂取カロリー実績の記録。いずれの概算値も「一般的な計算式による概算であり医学的指導ではない」旨を明記している。
  ...
  - 撤去した食事例の代わりに、**実際に食べた量を記録する「摂取カロリー実績」機能を追加(2026-08-28)**。朝食/昼食/夕食/間食ごとに、プロフィールに登録した「よく使うメニュー」を選び、**数量を指定して追加**できる(例: 焼酎1杯=100kcalを3杯選ぶと300kcalとして追加)。複数のメニューを積み上げて合計することも可能(例: 昼食=お弁当550kcal+ゆで卵120kcal=670kcal)。登録がないものはkcalを直接入力して追加することもでき、追加した項目は一覧表示され個別に削除できる。消費カロリー概算・摂取カロリー(実績・全食事の合計)・摂取カロリー目安(減量)を並べて表示し、「あと◯◯kcalまでOK」/「◯◯kcalオーバー」がひと目でわかるようにしている。推移の表示が必要になった場合もフェーズ2の反省を踏まえグラフ化はせず、数値・表形式で対応する方針。
...
- 初回導入時のみ、下記の初期リストを`ensureProfileSeeded()`が自動投入する(`mealItemsSeeded`フラグで一度きりの投入と管理し、ユーザーが後で全削除しても再投入されないようにしている)。

### 初期登録メニュー(2026-08-28時点)
...
| お寿司(1人前) | 550kcal |
| 牛乳1杯 | 130kcal |
| 手巻き(のり・魚・ひきわり納豆) | 1180kcal |
| 自作ピザ(肉) | 850kcal |
| 自作ピザ(しらす) | 750kcal |
| 白ワイン1杯 | 90kcal |
| チャーシュー200g | 480kcal |
| 三色丼(しらす・めかぶ・卵焼き) | 550kcal |
| 冷やし中華 | 500kcal |
| ラーメン(袋麺) | 470kcal |
| ラーメン(とんこつ・お店) | 700kcal |
| そば(ざる) | 320kcal |
| そば(カレーそば) | 480kcal |
